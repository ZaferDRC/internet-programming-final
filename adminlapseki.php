...
<body>
    ...
    if (isset($_POST["kaydet"])) {
        ...
        $sorgu = $baglandb->query("DELETE FROM lapseki");
        $sorgu = $baglandb->query("INSERT INTO lapseki (icerik) VALUES('$filtreicerik')");

        if ($sorgu) {

            echo "<script>window.location.href='adminlapseki.php';</script>";
        } else {
    ...
    $sorgu = $baglandb->query("SELECT * FROM lapseki");
    ...
    <form action="" method="POST">
        <h1 style="font-size: x-large; color:#fff;">LAPSEKİ İÇERİK:</h1>
