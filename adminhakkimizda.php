...
<body>
    ...
    if (isset($_POST["kaydet"])) {
        ...
        $icerik = $_POST["icerik"];
        $sorgu = $baglandb->query("DELETE FROM hakkimizda");
        $sorgu = $baglandb->query("INSERT INTO hakkimizda (icerik) VALUES('$icerik')");

        if ($sorgu) {

            echo "<script>window.location.href='adminhakkimizda.php';</script>";
        } else {
    ...
    $sorgu = $baglandb->query("SELECT * FROM hakkimizda");
    ...
    <form action="" method="POST">
        <h1 style="font-size: x-large; color:#fff;">HAKKIMIZDA İÇERİK:</h1>
        <textarea rows="20" cols="80" name="icerik"> <?php echo $satir->icerik; // Bu, veri tabanındaki icerik sütunu?></textarea><br><br>
