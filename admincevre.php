...
<body>
    ...
    if (isset($_POST["kaydet"])) {
        ...
        $icerik = $_POST["icerik"];
        $sorgu = $baglandb->query("DELETE FROM cevre");
        $sorgu = $baglandb->query("INSERT INTO cevre (icerik) VALUES('$icerik')");

        if ($sorgu) {

            echo "<script>window.location.href='admincevre.php';</script>";
        } else {
    ...
    $sorgu = $baglandb->query("SELECT * FROM cevre");
    ...
    <form action="" method="POST">
        <h1 style="font-size: x-large; color:#fff;">ÇEVRE İÇERİK:</h1>
