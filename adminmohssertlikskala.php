...
<body>
    ...
    if (isset($_POST["kaydet"])) {
        ...
        $sorgu = $baglandb->query("DELETE FROM mohs");
        $sorgu = $baglandb->query("INSERT INTO mohs (icerik) VALUES ('$filtreicerik')");

        if ($sorgu) {

            echo "<script>window.location.href='adminmohssertlikskala.php';</script>";
        } else {
    ...
    $sorgu = $baglandb->query("SELECT * FROM mohs");
    ...
    <form action="" method="POST">
        <h1 style="font-size: x-large; color:#fff;">MOHS SERTLİK İÇERİK:</h1>
