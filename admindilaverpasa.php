...
<body>
    ...
    if (isset($_POST["kaydet"])) {
        ...
        $sorgu = $baglandb->query("DELETE FROM dilaverpasa");
        $sorgu = $baglandb->query("INSERT INTO dilaverpasa (icerik) VALUES('$filtreicerik')");

        if ($sorgu) {

            echo "<script>window.location.href='admindilaverpasa.php';</script>";
        } else {
    ...
    $sorgu = $baglandb->query("SELECT * FROM dilaverpasa");
    ...
    <form action="" method="POST">
        <h1 style="font-size: x-large; color:#fff;">DİLAVER PAŞA İÇERİK:</h1>
