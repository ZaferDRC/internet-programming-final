...
<body>
    ...
    if (isset($_POST["kaydet"])) {
        ...
        $sorgu = $baglandb->query("DELETE FROM calisanaydinlatma");
        $sorgu = $baglandb->query("INSERT INTO calisanaydinlatma (icerik) VALUES('$filtreicerik')");

        if ($sorgu) {

            echo "<script>window.location.href='admincalisanaydinlatma.php';</script>";
        } else {
    ...
    $sorgu = $baglandb->query("SELECT * FROM calisanaydinlatma");
    ...
    <form action="" method="POST">
        <h1 style="font-size: x-large; color:#fff;">ÇALIŞAN AYDINLATMA İÇERİK:</h1>
