...
<body>
    ...
    if (isset($_POST["kaydet"])) {
        ...
        $sorgu = $baglandb->query("DELETE FROM issagligi");
        $sorgu = $baglandb->query("INSERT INTO issagligi (icerik) VALUES('$filtreicerik')");

        if ($sorgu) {

            echo "<script>window.location.href='adminissagligi.php';</script>";
        } else {
    ...
    $sorgu = $baglandb->query("SELECT * FROM issagligi");
    ...
    <form action="" method="POST">
        <h1 style="font-size: x-large; color:#fff;">İŞ SAĞLIĞI İÇERİK:</h1>
