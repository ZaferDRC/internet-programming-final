...
<body>
    ...
    if (isset($_POST["kaydet"])) {
        ...
        $sorgu = $baglandb->query("DELETE FROM madenhavalandirma");
        $sorgu = $baglandb->query("INSERT INTO madenhavalandirma (icerik) VALUES('$filtreicerik')");

        if ($sorgu) {

            echo "<script>window.location.href='adminmadenhava.php';</script>";
        } else {
    ...
    $sorgu = $baglandb->query("SELECT * FROM madenhavalandirma");
    ...
    <form action="" method="POST">
        <h1 style="font-size: x-large; color:#fff;">MADEN HAVALANDIRMA İÇERİK:</h1>
