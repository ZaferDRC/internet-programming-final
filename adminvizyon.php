...
<body>
    ...
    if (isset($_POST["kaydet"])) {
        ...
        $icerik = $_POST["icerik"];
        $sorgu = $baglandb->query("DELETE FROM vizyon");
        $sorgu = $baglandb->query("INSERT INTO vizyon (icerik) VALUES('$icerik')");

        if ($sorgu) {

            echo "<script>window.location.href='adminvizyon.php';</script>";
        } else {
    ...
    $sorgu = $baglandb->query("SELECT * FROM vizyon");
    ...
    <form action="" method="POST">
        <h1 style="font-size: x-large; color:#fff;">VİZYON İÇERİK:</h1>
