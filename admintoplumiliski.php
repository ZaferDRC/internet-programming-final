...
<body>
    ...
    if (isset($_POST["kaydet"])) {
        ...
        $sorgu = $baglandb->query("DELETE FROM toplumiliski");
        $sorgu = $baglandb->query("INSERT INTO toplumiliski (icerik) VALUES('$filtreicerik')");

        if ($sorgu) {

            echo "<script>window.location.href='admintoplumiliski.php';</script>";
        } else {
    ...
    $sorgu = $baglandb->query("SELECT * FROM toplumiliski");
    ...
    <form action="" method="POST">
        <h1 style="font-size: x-large; color:#fff;">TOPLUM İLİŞKİ İÇERİK:</h1>
