...
<body>
    ...
    if (isset($_POST["kaydet"])) {
        ...
        $sorgu = $baglandb->query("DELETE FROM mucevherat");
        $sorgu = $baglandb->query("INSERT INTO mucevherat (icerik) VALUES('$filtreicerik')");

        if ($sorgu) {

            echo "<script>window.location.href='adminmucevherat.php';</script>";
        } else {
    ...
    $sorgu = $baglandb->query("SELECT * FROM mucevherat");
    ...
    <form action="" method="POST">
        <h1 style="font-size: x-large; color:#fff;">MÜCEVHERAT İÇERİK:</h1>
